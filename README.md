...
- `off`. Defines the animation curve for the variable that doesn't change in the
  ...
  `scale` is a value to scale the global movement of the animation. As the curve
  is defined from `(0, 0)` to `(1, 0)`, the movement of the point is mapped onto
  those values, and `scale` can be used to reduce the amplitude	of the movement
  generated by the curve. A value of `1` will apply the curve without scaling,
  reduce this value if the displacement during animation is too big.
